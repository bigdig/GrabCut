...
void grabCut2( InputArray _img, InputOutputArray _mask, Rect rect,
...
{
    if( isInitialized )
        grabCut2( *image, mask, rect, bgdModel, fgdModel, 1, GC_EVAL );
    else
    {
        if( rectState != SET )
            return iterCount;
        ...
        if( lblsState == SET || prLblsState == SET )
            grabCut2( *image, mask, rect, bgdModel, fgdModel, 1, GC_INIT_WITH_MASK );
        else
            grabCut2( *image, mask, rect, bgdModel, fgdModel, 1, GC_INIT_WITH_RECT );
...
int main()
{
    string filename = "/Users/Photeinis/Desktop/gc/lena.bmp";
    if( filename.empty() )
    {
