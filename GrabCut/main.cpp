                             ...
                             GraphType* graph )
{
    ...
    for( int x = 0; x < img.rows; x++ )
    {
        for( int y = 0; y < img.cols; y++)
        {
            ...
            graph->add_tweights( nodeID, Souce, Sink );
            
            // set n-weights, the second item in the Gibbs formula
            if( y>0 )
            {
                double weight = leftWeight.at<double>(x,y);
                graph->add_edge( nodeID, nodeID-1, weight, weight );
            }
            if( x>0 && y>0 )
            {
                double weight = upleftWeight.at<double>(x,y);
                graph->add_edge( nodeID, nodeID-img.cols-1, weight, weight );
            }
            if( x>0 )
            {
                double weight = upWeight.at<double>(x,y);
                graph->add_edge( nodeID, nodeID-img.cols, weight, weight );
            }
            if( y<img.cols-1 && x>0 )
            {
...
static void estimateSegmentation( GraphType* graph, Mat& mask )
{
    ...
    for( int x = 0; x < mask.rows; x++ )
    {
        for( int y = 0; y < mask.cols; y++ )
        {
            if( mask.at<uchar>(x,y) == PBG || mask.at<uchar>(x,y) == PFG )
            {
                if( graph->what_segment( x*mask.cols+y ) == GraphType::SOURCE )
